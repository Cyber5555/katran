...
const ProductsInMainPage = ({ products, onRemoveFavorite }) => {
  ...
  const toggleFavorite = async (event, item) => {
    ...
    try {
      ...
      navbarFavoriteEffect.classList.add("shadow");
      setTimeout(() => {
        navbarFavoriteEffect.classList.remove("shadow");
      }, 2000);
    ...
    try {
      if (!isProductInBasket) {
        setIsBasket((prevBasket) => {
          ...
          //     dispatch(getBasketRequest({}));
          //     navbarBasketEffect.classList.add("shadow");
          //     setTimeout(() => {
          //       navbarBasketEffect.classList.remove("shadow");
          //     }, 2000);
          //   }
          // });
          // } else {
  ...
  // const calculatePrice = (item, count, userToken) => {
  //   const price = item?.new_price ? item.new_price * count : item.price * count;
  //   return userToken ? price : useNumberWithDots(price) + "֏";
  // };
  ...
  return (
    <div className="ProductsInMainPage__items__renderer--parent">
      {productData?.map((product) => (
        <div
          className="ProductsInMainPageFavorite__items--parent"
          key={product.id}
          onClick={() => {
